...
NH4_molar <- molar %>%
  filter(uptake_rate_name == "NH4_rate" & date != "2021-04-21") %>%
  ggplot(aes(x = temp_C, y = molar_rate)) +
  geom_point(aes(group = date_chamber, color = date_background)) +
  geom_smooth(aes(color = date_background), method = "lm") +
  scale_color_viridis_d("Experiment Date") +
...
SRP_molar <- molar %>%
  filter(uptake_rate_name == "SRP_rate") %>%
  ggplot(aes(x = temp_C, y = molar_rate)) +
  geom_point(aes(group = date_chamber, color = date_background)) +
  geom_smooth(aes(color = date_background), method = "lm") +
  scale_color_viridis_d("Experiment Date") +
...
mol_ratio_plot <- molar_ratio %>%
  filter(uptake_N_P_mol_ratio < 10 & uptake_N_P_mol_ratio > 0) %>% # filtering out outliers from aug 2021 experiment
...
UE_plot <- UE_long %>%
  ggplot(aes(x = temp_C, y = value, color = parameter, group = interaction(date, parameter))) +
  geom_point(aes(shape = date)) +
  geom_smooth(aes(color = parameter), method = "lm", se = FALSE) +
  labs(x = expression('Temperature (\u00B0C)'),
       y = expression(Nutrient~Use~Efficiency~(mol~O2:mol~N/P))) +
