...
amb_plot <- amb %>%
  ggplot(aes(x = date, y = concentration_ug_L, color = nutrient)) +
  geom_point() +
  geom_line() +
  scale_color_viridis_d(labels = expression(DP, SRP, NH[4]-N, NO[3]-N)) +
  labs(y = expression(Concentration~(paste(mu, "g")~L^{-1})), x = "Date", col = "Nutrient") +
  theme_classic()

amb_plot

ggsave("plots/nutrient_uptake/ambient_nutrients.png", plot = amb_plot, height = 4, width = 7, dpi = 300)
```
...
background_nut_uptake <- dat_long %>%
  ...
  scale_color_viridis_d("Experiment Date") +
  theme_bw() +
  labs(title = "Background Nutrient Uptake Rates vs. Temperature",
      x = expression('Temperature (\u00B0C)'),
      y = expression(Nutrient~Uptake~(ugN/ugP~hr^{-1})))
background_nut_uptake

ggsave("plots/nutrient_uptake/background_nut_uptake.png", plot = background_nut_uptake, height = 3, width = 7, dpi = 300)
```
...
control_uptake_plot <- control_uptake %>%
  ggplot(aes(x = date, y = avg_uptake, color = uptake_rate_name, group = uptake_rate_name)) +
  scale_color_viridis_d(name = "", labels = expression(NH[4], NO[3], SRP)) +
  geom_point() +
...
ggsave("plots/nutrient_uptake/control_nutrient_uptake_rate.png", plot = control_uptake_plot, height = 4, width = 7, dpi = 300)
```
...
NO3_plot <- combo %>%
  ggplot(aes(x = NO3_N_amb, y = avg_uptake, color = uptake_rate_name, group = uptake_rate_name)) +
 scale_color_viridis_d(name = "", labels = expression(NH[4], NO[3], SRP)) +
  geom_point() +
  geom_pointrange(aes(ymin = avg_uptake - sd_uptake, ymax = avg_uptake + sd_uptake)) +
  geom_line() +
  labs(x = expression(NH[4]~+~NO[3]~concentration~(paste(mu, "g")~L^{-1})), y = expression(Nutrient~Uptake~(paste(mu, "g")~N/paste(mu, "g")~P~gAFDW^{-1}~hr^{-1}))) +
  theme_classic()

# nutrient uptake rates as a function of ambient SRP concentrations
P_plot <- combo %>%
  ggplot(aes(x = SRP_amb, y = avg_uptake, color = uptake_rate_name, group = uptake_rate_name)) +
 scale_color_viridis_d(name = "", labels = expression(NH[4], NO[3], SRP)) +
  geom_point() +
  geom_pointrange(aes(ymin = avg_uptake - sd_uptake, ymax = avg_uptake + sd_uptake)) +
  geom_line() +
  labs(x = expression(SRP~concentration~(paste(mu, "g")~L^{-1})), y = expression(Nutrient~Uptake~(paste(mu, "g")~N/paste(mu, "g")~P~gAFDW^{-1}~hr^{-1}))) +
  theme_classic()

# nutrient uptake rates as a function of ambient NH4 concentrations

NH4_plot <- combo %>%
  ggplot(aes(x = NH4_N_amb, y = avg_uptake, color = uptake_rate_name, group = uptake_rate_name)) +
 scale_color_viridis_d(name = "", labels = expression(NH[4], NO[3], SRP)) +
  geom_point() +
  geom_pointrange(aes(ymin = avg_uptake - sd_uptake, ymax = avg_uptake + sd_uptake)) +
  geom_line() +
