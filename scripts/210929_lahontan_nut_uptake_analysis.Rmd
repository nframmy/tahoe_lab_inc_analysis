...
metab_dat <- metab_dat %>%
  filter(month != "october") %>% # filtering out october 2021 experiment since nutrient data is not available yet
  mutate(date = c(rep("2021-03-01", 16), rep("2021-03-15", 16), rep("2021-04-07", 16), rep("2021-04-21", 16), rep("2021-06-13", 32), rep("2021-08-08", 32))) %>% # creating date column
...
# plotting nutrient uptake rates
AFDW <- main %>%
  filter(date != "2021-04-21" & uptake_rate_name != "DP_rate") %>% # filtering out april 2021 data since rates were near zero due to error with nutrient enrichment, also filtering out DP data
...
#View(main)
molar <- main %>%
  filter(uptake_rate_name != "DP_rate") %>% # filtering out DP uptake rates since chemical there is no single chemical formula for this P species to calculate molar uptake rate with
  mutate(molar_rate = case_when(
    uptake_rate_name == "NH4_rate" ~ AFDW_rates/18.039,
    uptake_rate_name == "SRP_rate" ~ AFDW_rates/94.9714,
    uptake_rate_name == "NO3_rate" ~ AFDW_rates/62.0049
  ))
