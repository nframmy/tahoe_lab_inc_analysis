...
AFDW <- main %>%
  ...
  facet_grid(cols = vars(uptake_rate_name)) +
  geom_point(aes(group = date_chamber, color = date_background)) +
  geom_smooth(aes(color = date_background), method = "lm", se = F) +
  scale_color_viridis_d("Experiment Date") +
  theme_minimal() +
...
molar <- main %>%
  ...
  mutate(molar_rate = case_when(
    uptake_rate_name == "Ammonium" ~ AFDW_rates/14.0067e6,
    uptake_rate_name == "Soluble Reactive Phosphorus" ~ AFDW_rates/30.9738e6,
    uptake_rate_name == "Nitrate" ~ AFDW_rates/14.0067e6
  )) %>%
  mutate(molar_rate_d = molar_rate)
...
AFDW_molar <- molar %>%
  filter(date != "2021-04-21") %>% # removing april 2021 experiment data since nutrient enrichment conc. was too low
...
mol_ratio_plot <- molar_ratio %>%
  filter(uptake_N_P_mol_ratio < 10 & uptake_N_P_mol_ratio > 0 & date != "2021-04-21") %>% # filtering out outliers from aug 2021 experiment and april 2021 experiment data (nutrient levels were off)
  ggplot(aes(x = temp_C, y = uptake_N_P_mol_ratio)) +
  geom_point(aes(group = date_chamber, color = date)) +
  geom_smooth(aes(color = date), method = "lm", se = F) + # removing error ribbons from trendlines
  scale_color_viridis_d("Experiment Date") +
  theme_minimal() +
  theme(legend.position = "bottom") +
  labs(x = expression('Temperature (\u00B0C)'),
...
UE <- UE %>%
 mutate(NUE = (GPP_mg_d/1000/(15.999 * 2)) / (NH4_uptake_mol_d + NO3_uptake_mol_d), PUE = (GPP_mg_d/1000/(15.999 * 2)) / SRP_uptake_mol_d) %>% # converting GPP from mg O2 per day to mol O2 per day and then dividing by mol of N/P per day in order to calculate N and P use efficiency
  filter(!is.na(NUE))
...
UE %>%
  ggplot(aes(x = temp_C, y = NUE), group = date) +
  geom_point(aes(shape = date), color = "orange") +
  geom_smooth(aes(shape = date), method = "lm", color = "orange") +
  geom_point(aes(x = temp_C, y = PUE, shape = date), color = "purple") +
  geom_smooth(aes(x = temp_C, y = PUE, shape = date), method = "lm", color = "purple") +
  labs(x = expression('Temperature (\u00B0C)'),
...
UE_plot <- UE_long %>%
  ggplot(aes(x = temp_C, y = value, color = date, group = interaction(date, parameter))) +
  facet_grid(cols = vars(parameter)) +
  geom_point() +
  geom_smooth(aes(color = date), method = "lm", se = FALSE) +
  labs(x = expression('Temperature (\u00B0C)'),
       y = expression(mol~O2:mol~N/P)) +
...
ggsave("plots/nutrient_uptake/nutrient_use_eff.png", plot = UE_plot, height = 4, width = 6, dpi = 300)
