...
dat_clean <- dat_clean %>%
  mutate(AI = total_AFDW_g_2/(total_chla_ug/1e6))
```
...
AFDW_2_log_rates <- dat_long %>%
 ...
 geom_point(aes(shape = exp_ID, color = nutrient_trt), size = 1.5) +
  ...
  scale_color_viridis_d("Nutrient Treatment") + # renaming label of color legend
  scale_shape_discrete(name = "Experiment ID") +# renaming label of point shape legend
  scale_shape_manual(values = c(9, 15:20)) +
  theme_minimal() +
  theme(legend.position = "bottom") +
  labs(x = expression('Temperature (\u00B0C)'),
      y = expression(Rates~(mgO[2]~day^{-1}~gAFDW^{-1})))
...
# also adding it to dat_clean dataframe
dat_clean <- dat_clean %>%
  mutate(AI = total_AFDW_g_2/(total_chla_ug/1e6))
...
### GPP with AI and exp_ID added as fixed effects
```{r}
# making new dataset for lme models to remove NAs
dat_lme <- dat_clean %>%
...
# starting with as many explanatory variables as possible in the fixed component of the models (temp, nutrients, and their interaction)
m6 <- gls(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, method = "REML", data = dat_lme)

m7 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID, data = dat_lme) # adding in random intercept for each rock sample

m8 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme) # incorporating weights to account for heteroscedasticity within and btwn different experiments

m9 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID, data = dat_lme, control=(msMaxIter=500)) # adding in random intercept and slope for each rock sample, also increasing the number of iterations to acheive model convergence


#m10 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme, control=(msMaxIter=1000)) # incorporating weights to account for heteroscedasticity within and btwn different experiments, also increasing the number of iterations to acheive model convergence
# would not converge

# adding exp_ID as a fixed effect

m11 <- gls(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + exp_ID + AI, method = "REML", data = dat_lme)

m12 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + exp_ID + AI, random = ~ 1 | rock_ID, data = dat_lme) # adding in random intercept for each rock sample

m13 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + exp_ID + AI, random = ~ 1 | rock_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme) # incorporating weights to account for heteroscedasticity within and btwn different experiments

m14 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + exp_ID + AI, random = ~ 1 + temp_C | rock_ID, data = dat_lme, control=(msMaxIter=500)) # adding in random intercept and slope for each rock sample, also increasing the number of iterations to acheive model convergence


m15 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + exp_ID + AI, random = ~ 1 + temp_C | rock_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme, control=(msMaxIter=1000)) # incorporating weights to account for heteroscedasticity within and btwn different experiments, also increasing the number of iterations to acheive model convergence

# including exp_ID as a random effect instead (allowing temp & nutrient intercepts to vary by rock and experiment)

m16 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme)


AIC(m1, m2, m3, m4, m6, m7, m8, m9, m11, m12, m13, m14, m15, m16)
# model 3 has the lowest AIC

summary(m3)
# the model summary indicates that the nutrients and temp:nutrients interaction are not significant
...
# interpreting effect sizes of temp
exp(0.0458)
# for every 1C increase in temperature, GPP increases by 4.7% (a factor of 1.047 mgO2 per day per g AFDW).
```
...
plot(dat_lme$NEP_mgO2_d_gAFDW_2, E2)
head(E2)
```
...
# plotting model predicted values with actual data
# adding fitted values to dataframe
dat_lme$fitted <- fitted(m3)

# converting dataframe form wide to long format to plot fitted and actual data values on same plot
dat_lme <- dat_lme %>%
  mutate(exp_fitted = exp(fitted)) %>% #un-log-transforming fitted values so they can be plotted with raw data
  tidyr::pivot_longer(cols = c(exp_fitted, GPP_mgO2_d_gAFDW_2),
                               names_to = "data_type",
                               values_to = "value")

dat_lme %>%
  ggplot(aes(x = temp_C, y = value, color = data_type)) +
  scale_y_log10() +
  geom_point()

# estimating the temperature:nutrient effect using emmeans package

emmip(data = dat_lme, m3, nutrients ~ temp_C, cov.reduce = F)

emtrends(data = dat_lme, m3, pairwise ~ nutrients, var = "temp_C")
```
#### Graphical model validation
```{r}
# checking ordinary residuals of the final model (these are allowed to show heteroscedasticity). Using code from Zuur et al.
...
dat_lme <- dat_lme %>%
  mutate(exp_fitted = exp(fitted)) %>% #un-log-transforming fitted values so they can be plotted with raw data
  tidyr::pivot_longer(cols = c(exp_fitted, ER_mgO2_d_gAFDW_2),
                               names_to = "data_type",
                               values_to = "value")
...
# plotting model predicted values with actual data
# adding fitted values to dataframe
dat_lme$fitted <- fitted(m5)

# converting dataframe form wide to long format to plot fitted and actual data values on same plot
dat_lme <- dat_lme %>%
  mutate(exp_fitted = exp(fitted)) %>% #un-log-transforming fitted values so they can be plotted with raw data
  tidyr::pivot_longer(cols = c(exp_fitted, NEP_mgO2_d_gAFDW_2),
                               names_to = "data_type",
                               values_to = "value")
...
emmip(data = dat_lme, m5, nutrients ~ temp_C, cov.reduce = F)

emtrends(data = dat_lme, m5, pairwise ~ nutrients, var = "temp_C")
```

If I remove temp_C from the random effect (only allow for random intercepts instead for each rock_ID), the the residual plots look more normal and there isn't 1 experiment with uniform residuals of 0.
