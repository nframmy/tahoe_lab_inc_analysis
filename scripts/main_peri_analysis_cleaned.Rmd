...
chla_trends <- dat %>%
  ...
  geom_boxplot() +
  labs(x = "Date", y = expression(Periphyton~Chl-a~(g~m^{-2}))) +
  theme_classic()
...
metab_trend_sa <- dat_long_m2 %>%
  ...
  facet_grid(cols = vars(rate_name_m2)) +
  geom_boxplot() +
  labs(x = "", y = expression(GPP~(mgO[2]~day^{-1}~m^{2}))) +
  theme_classic()
...
ggsave("plots/main_analysis/metab_trends.png", plot = metab_trend, height = 8, width = 10, dpi = 300)
```
...
emtrends(GPP_final, pairwise ~ nutrient_trt | month, var = "temp_C")
...
emtrends(GPP_final, pairwise ~ nutrient_trt | month, var = "temp_C") # checking out temperature-nutrient interactions for each month
