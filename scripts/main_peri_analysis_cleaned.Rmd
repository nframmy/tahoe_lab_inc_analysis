...
chla_trends <- dat %>%
  mutate(chla_ug_m2 = total_chla_ug/rock_SA_m2) %>% # calculating areal chlorophyll-a concentrations (per m2)
  ggplot(aes(x = date, y = chla_ug_m2, group = date)) +
  geom_boxplot() +
  labs(x = "Date", y = "Periphyton Total Chlorophyll-a (ug/m2)") +
  theme_classic()
...
AFDW_trends <- dat %>%
  ...
  ggplot(aes(x = date, y = total_AFDW_g_2, group = date)) +
  geom_boxplot() +
  labs(x = "Date", y = expression(Periphyton~AFDW~(g~m^{-2}))) +
  theme_classic()
...
m5 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI + exp_ID, random = ~ 1 + temp_C | rock_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme, control=(msMaxIter=1000)) # incorporating weights to account for heteroscedasticity within and btwn different experiments, also increasing the number of iterations to acheive model convergence
...
m6 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, data = dat_lme)

m7 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme)

m8 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID / exp_ID, data = dat_lme, control=(msMaxIter=500))

#m9 <- m5 <- lme(log(GPP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme, control=(msMaxIter=1000))
# does not converge
...
summary(m5)
# the model summary indicates that temp and exp_ID7 are significant predictors of GPP
...
exp(0.043)
# for every 1C increase in temperature, GPP increases by 4.4% (a factor of 1.044 mgO2 per day per g AFDW).
```
...
m6 <- lme(log(ER_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, data = dat_lme)

m7 <- lme(log(ER_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme)

#m8 <- lme(log(ER_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID / exp_ID, data = dat_lme)
# does not converge

#m9 <- lme(log(ER_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme)
# does not converge


AIC(m1, m2, m3, m4, m6, m7)
...
m6 <- lme(log(NEP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, data = dat_lme)

m7 <- lme(log(NEP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme)

m8 <- lme(log(NEP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C| rock_ID / exp_ID, data = dat_lme, control=(msMaxIter=500))

m9 <- lme(log(NEP_mgO2_d_gAFDW_2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C| rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme, control=(msMaxIter=500))
...
# interpreting temp effect size in m7
exp(0.036)

# for every 1C increase in temperature, NEP increases by 3.7% (a factor of 1.037 mg O2 per day per g AFDW)
```
...
m6 <- lme(log(GPP_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, data = dat_lme)

m7 <- lme(log(GPP_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme)

m8 <- lme(log(GPP_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID / exp_ID, data = dat_lme, control=(msMaxIter=500))

m9 <- lme(log(GPP_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme, control=(msMaxIter=1000))
# does not converge
...
m6 <- lme(log(ER_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, data = dat_lme)

m7 <- lme(log(ER_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme)

m8 <- lme(log(ER_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID / exp_ID, data = dat_lme)
# does not converge

m9 <- lme(log(ER_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme)
# does not converge
...
m6 <- lme(log(NEP_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, data = dat_lme)

m7 <- lme(log(NEP_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 | rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme)

m8 <- lme(log(NEP_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C| rock_ID / exp_ID, data = dat_lme, control=(msMaxIter=500))

m9 <- lme(log(NEP_mgO2_d_m2) ~ temp_C * nutrients + AI, random = ~ 1 + temp_C| rock_ID / exp_ID, weights = varIdent(form = ~1|exp_ID), data = dat_lme, control=(msMaxIter=500))
