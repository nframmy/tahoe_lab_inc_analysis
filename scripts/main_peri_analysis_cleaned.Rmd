...
AIC(m1, m2, m3, m4, m5, m6, m7, m9, m10)
...
exp(0.045)
# for every 1C increase in temperature, GPP increases by 4.6% (a factor of 1.046 mgO2 per day per g AFDW).
...
tab_model(m1, m2, m3, m4, m5, m6, m7, m9, dv.labels = c("Model1", "Model2", "Model3", "Model4", "Model5", "Model6", "Model7", "Model9"),
  show.ci = FALSE, 
  p.style = "stars", 
  transform = NULL)
```
...
# diagnostic plots for model with exp_ID included as fixed effect for comparison of residuals
# checking ordinary residuals of the final model (these are allowed to show heteroscedasticity). Using code from Zuur et al.
E3 <- resid(m10)
...
emtrends(m5, pairwise ~ nutrients|exp_ID, var = "temp_C")
...
rg<- emmip(m5, nutrients ~ temp_C|exp_ID, CIs = T, cov.keep = c("temp_C", "nutrients"))


# same interaction analysis for m10 (has month instead of exp_ID in model)

emmeans(m10, pairwise ~ nutrient_trt|month, cov.keep = "temp_C")
...
emmeans(m10, pairwise ~ month, cov.keep = "temp_C")
...
GPP_IP <- emmip(m10, nutrient_trt ~ temp_C|month, CIs = T, cov.keep = c("temp_C", "nutrient_trt")) +
  geom_point(aes(x = temp_C, y = log(GPP_mgO2_d_gAFDW_2), color = nutrient_trt), data = dat_lme) +
...
emmip(m10, nutrient_trt ~ temp_C|month, CIs = T, cov.keep = c("temp_C", "nutrient_trt"), type = "response") +
...
# checking ordinary residuals of the final model (these are allowed to show heteroscedasticity). Using code from Zuur et al.
E3 <- resid(m3)
coplot(E3 ∼ rock_ID|exp_ID,
ylab = "Ordinary residuals", data = dat_lme)

# now checking standardized residuals of each of the experiments (random effect in the model). These should not show any patterns and should exhibit homogeneous variance.
E4 <- resid(m3, type = "normalized")
coplot(E4 ∼ rock_ID|exp_ID,
ylab = "Normalized residuals", data = dat_lme)

plot(m3)


# plots of model 10 with month as fixed effect and contrast levels readjusted

E5 <- resid(m10)
coplot(E5 ∼ rock_ID|month,
ylab = "Ordinary residuals", data = dat_lme)

# now checking standardized residuals of each of the experiments (random effect in the model). These should not show any patterns and should exhibit homogeneous variance.
E6 <- resid(m10, type = "normalized")
coplot(E6 ∼ rock_ID|month,
ylab = "Normalized residuals", data = dat_lme)
...
# interaction plot of temp, nutrients, and month in log scale
ER_IP <- emmip(m10, nutrient_trt ~ temp_C|month, CIs = T, cov.keep = c("temp_C", "nutrient_trt")) +
...
tab_model(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10, dv.labels = c("Model1", "Model2", "Model3", "Model4", "Model5", "Model6", "Model7", "Model8", "Model9"),
  show.ci = FALSE, 
  p.style = "stars", 
  transform = NULL)
```
...
# interaction plot of temp, nutrients, and month in log scale
NEP_IP <- emmip(m10, nutrient_trt ~ temp_C|month, CIs = T, cov.keep = c("temp_C", "nutrient_trt")) +
