...
# checking to see if numbers of samples add up
str(main_dat)
length(which(main_dat$NO3_rate != "NA")) # number of measurements for nitrate uptake rates
length(which(main_dat$NH4_rate != "NA"))
length(which(main_dat$SRP_rate != "NA"))
length(which(main_dat$DP_rate != "NA"))

length(which(main_dat$NO3_N_ug_L_initial
 != "NA")) + length(which(main_dat$NO3_N_final
 != "NA")) # number of measurements of nitrate

length(which(main_dat$NH4_N_ug_L_initial
 != "NA")) + length(which(main_dat$NH4_N_final
 != "NA")) # number of measurements of nitrate

length(which(main_dat$SRP_ug_L_initial
 != "NA")) + length(which(main_dat$SRP_final
 != "NA")) # number of measurements of nitrate

length(which(main_dat$DP_ug_L_initial
 != "NA")) + length(which(main_dat$DP_final
 != "NA")) # number of measurements of nitrate

length(which(main_dat$periphyton_PP_ug_L != "NA")) + length(which(main_dat$periphyton_PP_ug_g
 != "NA"))

length(which(main_dat$total_DW_g
 != "NA")) # number of measurements of nitrate

length(which(main_dat$total_AFDW_g_2
 != "NA")) # number of measurements of nitrate

length(which(main_dat$total_chla_ug
 != "NA")) # number of measurements of nitrate

length(unique(main_dat$rock_ID)) # number of different rocks
...
# looking at GPP grouped by month
...
dat_clean %>%
  ...
  mutate(grand_mean = mean(GPP_mgO2_d_gAFDW_2)) %>% 
  group_by(month) %>% 
  mutate(month_mean = mean(GPP_mgO2_d_gAFDW_2)) %>% 
  ggplot(aes(x = month, y = GPP_mgO2_d_gAFDW_2)) +
  geom_boxplot() +
  geom_label(aes(x = month, y = month_mean + 50, label = paste(month_mean))) +
  geom_label(aes(x = "october", y = grand_mean + 50, label = paste(grand_mean)))
...
chla_trends <- dat %>%
  ...
  mutate(chla_mg_m2 = total_chla_ug/rock_SA_m2/1e3) %>% # calculating areal chlorophyll-a concentrations (per m2)
  ggplot(aes(x = date, y = chla_mg_m2, group = date)) +
  geom_boxplot() +
  labs(x = "", y = expression(Periphyton~Chl-a~(mg~m^{-2}))) +
...
AI_AFDW <- (chla_trends/AFDW_trends/AI_trends/insitu_temp)

ggsave("plots/main_analysis/AI_AFDW_trends.jpg", plot = AI_AFDW, height = 11, width = 11, dpi = 300)

ggsave("plots/final_paper/Figure2.jpg", plot = AI_AFDW, height = 10, width = 10, dpi = 600)
```
...
All values fall within the extreme P deficiency group as put forth by Pick & Lean 1981.
...
control_rates <- dat_long_m2 %>%
  ...
  summarise(avg_rate_m2 = mean(rates_m2))
#View(control_rates)
```
...
ggsave("plots/final_paper/Figure3.jpg", plot = metab_trend, height = 6, width = 12, dpi = 300)
...
gt::gtsave("plots/main_analysis/nut_seasonal_GPP.png", data = table)
...
GPP_IP <- emmip(GPP_final, nutrient_trt ~ temp_C|month, CIs = TRUE, cov.keep = c("temp_C", "nutrient_trt")) +
...
emmip(GPP_final, nutrient_trt ~ temp_C|month, CIs = T, cov.keep = c("temp_C", "nutrient_trt"), type = "response") +
  geom_point(aes(x = temp_C, y = GPP_mgO2_d_gAFDW_2, color = nutrient_trt), data = dat_lme, inherit.aes = FALSE) +
  theme_minimal()
...
temp_nut_table_ER <- temp_nut_seasonal_ER %>%
  mutate(estimate = exp(estimate), SE = exp(SE)) %>% # converting temp:nut interaction estimate to the response scale due to the log-transformation
  gt() %>%
...
emmip(ER_final, nutrient_trt ~ temp_C|month, CIs = T, cov.keep = c("temp_C", "nutrient_trt"), type = "response") +
  geom_point(aes(x = temp_C, y = ER_mgO2_d_gAFDW_2, color = nutrient_trt), data = dat_lme, inherit.aes = FALSE) +
  theme_minimal()
```
...
emmip(NEP_final, nutrient_trt ~ temp_C|month, CIs = T, cov.keep = c("temp_C", "nutrient_trt"), type = "response") +
  geom_point(aes(x = temp_C, y = NEP_mgO2_d_gAFDW_2, color = nutrient_trt), data = dat_lme, inherit.aes = FALSE) +
  facet_grid(cols = vars(month)) +
  theme_minimal()
```
...
tab_model(GPP_final, ER_final, NEP_final, dv.labels = c("GPP", "ER", "NEP"),
  ...
  p.style = "stars", 
  transform = "exp",
  file = "plots/final_paper/Table2.doc")
```
