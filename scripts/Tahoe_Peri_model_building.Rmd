...
# hourly rates
GPP_IP_hr <- emmip(GPP_final_hr, nutrient_trt ~ temp_C|month, CIs = FALSE, cov.keep = c("temp_C", "nutrient_trt")) +
  geom_point(aes(x = temp_C, y = log(GPP_mgO2_hr_gAFDW_2), color = nutrient_trt), data = dat_lme, inherit.aes = FALSE) +
  facet_grid(cols = vars(month)) +
  labs(y = expression(log(GPP~(mgO[2]~hr^{-1}~gAFDW^{-1}))), x = "") +
...
# hourly rates
ER_IP_hr <- emmip(ER_final_hr, nutrient_trt ~ temp_C|month, CIs = FALSE, cov.keep = c("temp_C", "nutrient_trt")) +
  geom_point(aes(x = temp_C, y = log(ER_mgO2_hr_gAFDW_2), color = nutrient_trt), data = dat_lme, inherit.aes = FALSE) +
  facet_grid(cols = vars(month)) +
  labs(y = expression(log(ER~(mgO[2]~hr^{-1}~gAFDW^{-1}))), x = "") +
  scale_color_manual(values = c("#F8766D", "#00BFC4")) +
  theme_bw() +
  theme(legend.position = "none")
```
...
# hourly rates
NEP_IP_hr <- emmip(NEP_final_hr, nutrient_trt ~ temp_C|month, CIs = FALSE, cov.keep = c("temp_C", "nutrient_trt")) +
  geom_point(aes(x = temp_C, y = log(NEP_mgO2_hr_gAFDW_2), color = nutrient_trt), data = dat_lme, inherit.aes = FALSE) +
  facet_grid(cols = vars(month)) +
  labs(y = expression(log(NEP~(mgO[2]~hr^{-1}~gAFDW^{-1}))), x = expression('Temperature (\u00B0C)')) +
