...
gtsave("plots/final_paper/Table2A.docx", data = nut_seasonal_GPP)
...
temp_GPP <- emtrends(GPP_final, ~ temp_C, var = "temp_C", type = "response") %>% 
  data.frame() %>% 
  mutate_at(c("temp_C.trend", "lower.CL", "upper.CL"), exp)
...
temp_emmeans_table <- temp_seasonal_emm %>%
  mutate_at(c("temp_C.trend", "lower.CL", "upper.CL"), exp) %>% # putting the estimated temp trends and CIs in the response scale by undoing the log-transformation
...
gtsave("plots/final_paper/Table1_GPP.docx", data = temp_emmeans_table)
...
# if you want model results to be presented using sum contrasts then use this code
ER_contr_sum <- update(ER_final, contrasts = list(month = contr.sum, nutrient_trt = contr.sum)) # updating contrast matrix of the final model summary
summary(ER_contr_sum)
...
nut_seasonal_ER <- nut_seasonal_ER$contrasts %>%
  ...
  cols_label(ratio = "Effect Size") %>%
  tab_header(title = "ER")
gtsave("plots/final_paper/Table2B.docx", data = nut_seasonal_ER)
...
######################
# overall effect of temperature averaged over nutrient_trt and months
temp_ER <- emtrends(ER_final, ~ temp_C, var = "temp_C", type = "response") %>% 
  data.frame() %>% 
  mutate_at(c("temp_C.trend", "lower.CL", "upper.CL"), exp)
...
temp_emmeans_table_ER <- temp_seasonal_ER_emm %>%
   mutate_at(c("temp_C.trend", "lower.CL", "upper.CL"), exp) %>% # putting the estimated temp trends in the response scale by undoing the log-transformation
   mutate(Q10_value = temp_C.trend^10) %>% # calculating Q10 values
   mutate(across(c(temp_C.trend:Q10_value), ~round(.,2))) %>% # rounding all numeric columns to 4 sig figs
    gt() %>%
  cols_move(columns = Q10_value, after = temp_C.trend) %>%
...
gtsave("plots/final_paper/Table1_ER.docx", data = temp_emmeans_table_ER)
...
# if you want model results to be presented using sum contrasts then use this code
NEP_contr_sum <- update(NEP_final, contrasts = list(month = contr.sum, nutrient_trt = contr.sum)) # updating contrast matrix of the final model summary


summary(NEP_contr_sum)
...
gtsave("plots/final_paper/Table2C.docx", data = nut_seasonal_NEP)
...
###################
# overall effect of temperature averaged over nutrient_trt and months
temp_NEP <- emtrends(NEP_final, ~ temp_C, var = "temp_C", type = "response") %>% 
  data.frame() %>% 
  mutate_at(c("temp_C.trend", "lower.CL", "upper.CL"), exp)
...
# saving table for final paper
gtsave("plots/final_paper/Table1_NEP.docx", data = temp_emmeans_table_NEP)
...
tab_model(GPP_final, ER_final, NEP_final, dv.labels = c("GPP", "ER", "NEP"),
  ...
  p.style = "stars", 
  transform = "exp",
  file = "plots/final_paper/appendix5.doc")
...
tab_model(GPP_contr_sum, ER_contr_sum, NEP_contr_sum, dv.labels = c("GPP", "ER", "NEP"),
  show.ci = FALSE, 
  p.style = "stars", 
  transform = "exp",
  file = "plots/final_paper/appendix5_sum_contr.doc")
```



##### factor levels of contrast sums model summary
```{r}
grand_means <- dat_lme %>% 
  summarise(grand_mean_GPP = mean(log(GPP_mgO2_d_gAFDW_2)), grand_mean_ER = mean(log(ER_mgO2_d_gAFDW_2)), grand_mean_NEP = mean(log(NEP_mgO2_d_gAFDW_2)))

dat_lme %>% 
  group_by(month) %>% 
  summarise(avg_GPP = mean(log(GPP_mgO2_d_gAFDW_2)), sd_GPP = sd(log(GPP_mgO2_d_gAFDW_2))) %>% 
  ggplot(aes(x = month, y = avg_GPP))+
  geom_point()+
  geom_errorbar(aes(ymin=avg_GPP-sd_GPP, ymax=avg_GPP+sd_GPP), width=.2, position=position_dodge(.9)) +  
  geom_hline(yintercept = grand_means$grand_mean_GPP)

dat_lme %>% 
  group_by(month) %>% 
  summarise(avg_GPP = mean(log(GPP_mgO2_d_gAFDW_2)), sd_GPP = sd(log(GPP_mgO2_d_gAFDW_2))) %>% 
  summarise(grand_mean_GPP = mean(avg_GPP))


# calculating unweighted mean of GPP and seeing if this equals the grand mean in the intercept value
u_mean_GPP <- dat_lme %>% 
  group_by(month, nutrient_trt) %>% 
  summarise(group_avg_GPP = mean(log(GPP_mgO2_d_gAFDW_2)), n = n())
u_mean_GPP

# sample size matches up
sum(u_mean_GPP$n)

u_mean_GPP %>% 
  ungroup() %>% 
  summarise(grand_unweighted_mean_GPP = mean(group_avg_GPP), n = n())
...
gtsave("plots/final_paper/Appendix11_GPP.docx", data = GPP)
...
gtsave("plots/final_paper/Appendix11_ER.docx", data = ER)
...
gtsave("plots/final_paper/Appendix11_NEP.docx", data = NEP)
