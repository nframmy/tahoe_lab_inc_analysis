...
# Get all combinations of month, nutrient_trt, and temp_C within each month's observed range
# pred_grid <- dat_lme %>%
...
#   ) %>%
#   rowwise() %>%
#   mutate(temp_seq = list(seq(min_temp, max_temp, length.out = 50))) %>%
#   unnest(temp_seq) %>%
#   rename(temp_C = temp_seq)
# 
# # Create prediction dataset
# newdat <- pred_grid %>%
#   dplyr::select(temp_C, nutrient_trt, month)
# 
# # Predict using emmeans/ref_grid
# ref <- ref_grid(GPP_final_hr, data = dat_lme, 
#                 cov.reduce = FALSE)  # don't reduce covariate range
# # Force the prediction vector into a data frame with a proper column name
# preds_df <- data.frame(fit = predict(ref, newdata = newdat, type = "response"))
# 
# # Now combine safely
# pred_plot_data <- cbind(newdat, preds_df$fit)
# 
# # Plot manually
# ggplot() +
#   geom_line(
#     data = pred_plot_data,
#     aes(x = temp_C, y = fit, color = nutrient_trt)
#   ) +
#   geom_point(
#     data = dat_lme,
#     aes(x = temp_C, y = GPP_mgO2_hr_gAFDW_2, color = nutrient_trt)
#   ) +
#   scale_y_log10() +
#   facet_grid(cols = vars(month)) +
#   labs(y = expression(GPP~(mgO[2]~hr^{-1}~gAFDW^{-1})), x = "") +
