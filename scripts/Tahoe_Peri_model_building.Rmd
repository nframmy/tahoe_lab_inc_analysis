...
tab_model(GPP_contr_sum, ER_contr_sum, NEP_contr_sum, dv.labels = c("GPP", "ER", "NEP"),
  ...
  p.style = "stars", 
  transform = "exp",
  file = "plots/final_paper/Table1_sum_contr.doc")
```
...
```{r}
grand_means <- dat_lme %>% 
  summarise(grand_mean_GPP = mean(GPP_mgO2_d_gAFDW_2), grand_mean_ER = mean(ER_mgO2_d_gAFDW_2), grand_mean_NEP = mean(NEP_mgO2_d_gAFDW_2))

dat_lme %>% 
  group_by(month) %>% 
  summarise(avg_GPP = mean(GPP_mgO2_d_gAFDW_2), sd_GPP = sd(GPP_mgO2_d_gAFDW_2)) %>% 
