...
dat_ER <- dat2 %>%
  filter(time >= hms::as.hms("10:07:00")) %>% # filtering out initial DO msmnt to only get NEP
  group_by(jar) %>%
  summarise(treatment, tank, temp, DO_diff_mg_L = first(DO_avg) - last(DO_avg), delta_T_days = as.duration(first(time) %--% last(time))/ddays(1)) # taking difference in DO conc. between pre- and post-dark exposure and difference in time (in days) to calculate ER rates
